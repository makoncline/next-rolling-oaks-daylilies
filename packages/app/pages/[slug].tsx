              ...
              {listings &&
                ...
                Array.from(
                  new Set(
                    listings
                      .filter(
                        (lily: Listing) => lily.ahs_data && lily.ahs_data.ploidy
                      )
                      .map(
                        (lily: Listing) => lily.ahs_data && lily.ahs_data.ploidy
                      )
            ...
            >
              ...
              {listings &&
                ...
                Array.from(
                  ...
                  .sort((a, b) => sortAlphaNum(a + "", b + ""))
                  .map((fragrance, i) => (
                    <option key={"fragrance-" + i} value={fragrance as string}>
                      {fragrance}
                    </option>
                  ))}
            </Select>
...
export async function getStaticPaths() {
  ...
  paths.push({ params: { slug: "search" } });
  paths.push({ params: { slug: "for-sale" } });
  return {
    paths: paths,
    fallback: false,
  };
...
export async function getStaticProps(context: any) {
  ...
  if (slug === "for-sale") {
    ...
    const listId = listIds.find((node) => slugify(node.name) === slug)?.id;
